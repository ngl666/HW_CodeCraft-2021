...
//            addvms
            ...
            ArrayList<ArrayList<ArrayList<String>>> theseDaysAddVmTemporaryPart = new ArrayList<>();
            int t=0;
            if(theseDaysAddVmTemporaryCopy.size()>500){
                while (t<theseDaysAddVmTemporaryCopy.size()) {

                    if ((t + 500) > theseDaysAddVmTemporaryCopy.size()) {

                        theseDaysAddVmTemporaryPart.add(new ArrayList<>(theseDaysAddVmTemporaryCopy.subList(t, theseDaysAddVmTemporaryCopy.size())));
                    } else {
                        theseDaysAddVmTemporaryPart.add(new ArrayList<>(theseDaysAddVmTemporaryCopy.subList(t, t + 500)));
                    }
                    t = t + 500;
                }
            }else {
            ...
            for(int a=0;a<theseDaysAddVmTemporaryPart.size();a++){

                ArrayList<ArrayList<String>>   theseDaysAddVmTemporary = theseDaysAddVmTemporaryPart.get(a);

                while (!theseDaysAddVmTemporary.isEmpty()){
//                    System.out.println(theseDaysAddVmTemporary.size());

                    for(int j=0;j<serverCostPerformanceOrder.size();j++){
                        String servername =serverCostPerformanceOrder.get(j);
                        Server serverObject = allServerObject.get(servername);
                        ...
                        int  node1serverCpu = oneServer.getNum_cpu()/2;
                        int  node1serverRam = oneServer.getRAM()/2;
                        int c =node1serverCpu;
                        int d =node1serverRam;

                        int iter =theseDaysAddVmTemporary.size();

                        int[] v = new int[iter];//价值，cpu+ram 双节点的/2
                        int[] w = new int[iter];//cpu 双节点的/2
                        int[] b = new int[iter];//ram 双节点的/2
                        for(int i1=0;i1<iter;i1++){
                            VM vm= allVmObject.get(theseDaysAddVmTemporary.get(i1).get(1));
                            if(vm.getNodetype() ==0){
                                w[i1] = vm.getNum_cpu();
                                b[i1] = vm.getRAM();
                                v[i1] = w[i1]+b[i1];
                        ...
                        ArrayList<Integer> bestVmForServerId=dynamicProgram(v,w,b,c,d);
                        for(int k=0;k<bestVmForServerId.size();k++){
                            int id =bestVmForServerId.get(k);
                            VM vm = currentVm.get(Integer.parseInt(theseDaysAddVmTemporary.get(id).get(3)));
                            if(vm.getNodetype()==0){
                                oneServer =arrangeVm(vm,oneServer,1);
                                theseDaysAddVmTemporary.remove(id);
                            }else {
                        ...
                        int c1 =oneServer.getNode2().getNum_cpu()-oneServer.getNode2().getCpuisoccupied();
                        int d1 =oneServer.getNode2().getRAM() - oneServer.getNode2().getRAMisoccupied();
                        //二节点时只能部署单节点vm<计算出来的id,theseDaysAddVmTemporary的id>
                        HashMap<Integer,Integer> maping = new HashMap<>();
                        int iter1 =0;
                        for(int q=0;q<theseDaysAddVmTemporary.size();q++){
                            VM vm= allVmObject.get(theseDaysAddVmTemporary.get(q).get(1));
                            if(vm.getNodetype() ==0 ){
                                maping.put(maping.size(),q);
                                iter1++;
                            }
                        }
                        int[] v1 = new int[iter1];//价值，cpu+ram 双节点的/2
                        int[] w1 = new int[iter1];//cpu 双节点的/2
                        int[] b1 = new int[iter1];//ram 双节点的/2
                        for(int i1=0;i1<iter1;i1++){
                            VM vm= allVmObject.get(theseDaysAddVmTemporary.get(maping.get(i1)).get(1));
                            //只计算单节点
                            if(vm.getNodetype() ==0){
                                w1[i1] = vm.getNum_cpu();
                                b1[i1] = vm.getRAM();
                        ...
                        ArrayList<Integer> bestVmForServerId1=dynamicProgram(v1,w1,b1,c1,d1);
                        for(int k=0;k<bestVmForServerId1.size();k++) {
                            int id =maping.get(bestVmForServerId1.get(k));
                            VM vm = currentVm.get(Integer.parseInt(theseDaysAddVmTemporary.get(id).get(3)));
                            if (vm.getNodetype() == 0) {
...
//                for(Integer vmId :oneServer.getNode1().getVmlist()){
//                    Integer vmType = currentVm.get(vmId).getNodetype();
//                    if(vmType == 0){
//                        ArrayList<Integer> vmInserver = new ArrayList<>();
//                        vmInserver.add(index);
//                        vmInserver.add(1);
//                        VMidInServer.put(vmId,vmInserver);
//                    }else {
//                        ArrayList<Integer> vmInserver = new ArrayList<>();
//                        vmInserver.add(index);
//                        vmInserver.add(3);
...
//
//                }
//                for(Integer vmId :oneServer.getNode2().getVmlist()){
//                    Integer vmType = currentVm.get(vmId).getNodetype();
//                    if(vmType == 0){
//                        ArrayList<Integer> vmInserver = new ArrayList<>();
//                        vmInserver.add(index);
...
//                }
                        ...
                        if(theseDaysAddVmTemporary.isEmpty()){
                            break;
                        }
...
//
//
//            int canMoveNumber = (int) Math.floor(currentVm.size() * 5.0 / 1000);
//
//
//            //输出迁移方案
//            for (int z = 0; z < moveInfo.size(); z++) {
